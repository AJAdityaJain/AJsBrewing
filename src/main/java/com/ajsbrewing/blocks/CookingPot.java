...
import static net.minecraft.particle.BlockStateParticleEffect.createCodec;
...
//    public static final MapCodec<CookingPot> CODEC = createCodec(CookingPot::new);


//    public MapCodec<CookingPot> getCodec() {
//        return CODEC;
//    }
    ...
    public void onEntityCollision(BlockState state, World world, BlockPos pos, Entity entity) {
        if (entity instanceof ItemEntity itemEntity) {
            ...
            if (blockEntity instanceof CookingPotEntity cookingPotEntity) {
                if (state.get(FILLED)) {
                    BlockState neighbor = world.getBlockState(pos.down());
                    if (isPreparer(neighbor)) {
                        cookingPotEntity.setPreparer(neighbor);

                        cookingPotEntity.onEntityCollided(world, pos, state, itemEntity);
                    }
                }
            }
        }
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        ...
        if (world.getBlockEntity(pos) instanceof CookingPotEntity cookingPotEntity) {
            ItemStack item = player.getStackInHand(hand);
            if (state.get(FILLED)) {
                if (item.isOf(EmptyVialItem.INSTANCE)) {
                    Hand hand2 = hand == Hand.MAIN_HAND ? Hand.OFF_HAND : Hand.MAIN_HAND;
                    ItemStack item2 = player.getStackInHand(hand2);

                    if (player.getStackInHand(hand2).isOf(WitchcraftTome.INSTANCE)) {
                        NbtCompound nbt = item2.getOrCreateNbt();
                        NbtCompound nbt2 = new NbtCompound();
                        cookingPotEntity.writeNbt(nbt2);
                        NbtList nbtl = nbt.getList("recipes", NbtElement.COMPOUND_TYPE);
                        nbtl.add(nbt2);
                        nbt.put("recipes", nbtl);
                        AJsBrewingMod.LOGGER.info("Added recipe to tome : " + nbt2.toString());
                    }

                    if (isPreparer(world.getBlockState(pos.down()))) {
                        item.decrement(1);
                        player.giveItemStack(cookingPotEntity.getPotionItem());
                ...
                cookingPotEntity.setColor(world.getBiome(pos).value().getWaterColor());
                return ActionResult.CONSUME;
            }
        }
    ...
    public boolean isPreparer(BlockState state) {
